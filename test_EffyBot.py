...
'''@dp.message( Text(text = back_list), StateFilter(FSM.lang_pick), Command(commands = ['cancel']))
async def lang_back_process(message: Message, state = FSMContext):
    chosen_lang = lang_choice(message.from_user.id)
    await message.answer(lang['settings_'][f'{chosen_lang}'], reply_markup = kbs[chosen_lang]['cfg_kb'])
    await state.set_state(FSM.settings_menu)

@dp.message(State(FSM.settings_menu), Text(text = settings_list), Command(commands = ['cancel']))
async def settings_back_process(message: Message, state = FSMContext):
    chosen_lang = lang_choice(message.from_user.id)
    await message.answer(lang['welcome_'][f'{chosen_lang}'], reply_markup = kbs[chosen_lang]['main_menu_kb'])
...
''', StateFilter(FSM.ask_create_dialogue),''' 
